...
class HierarchicalStateSpaceModel(nn.Module):
    def __init__(self,
        ...
        #################################
        self.enc_obs = Encoder(output_size=self.feat_size,
                               feat_size=self.feat_size)
        self.dec_obs = Decoder(input_size=self.feat_size,
                               feat_size=self.feat_size)
    ...
    def random_index_from_ones(self, read_data_list, init_size, seq_size):
        # Create a mask for elements that are 1
        mask = (read_data_list == 1)
        range_mask = (torch.arange(read_data_list.size(1)).unsqueeze(0).expand_as(read_data_list) >= init_size) & \
                    (torch.arange(read_data_list.size(1)).unsqueeze(0).expand_as(read_data_list) < init_size + seq_size)
        mask &= range_mask.to(mask.device)
        ...
        result = [None] * read_data_list.size(0)

        # Iterate over unique row indices and randomly select a column index
        for row in torch.unique(row_indices):
            row_mask = row_indices == row
            valid_cols = col_indices[row_mask]
            if valid_cols.size(0) > 0:
                chosen_col = valid_cols[torch.randint(0, valid_cols.size(0), (1,)).item()]
                result[row.item()] = chosen_col.item()

        return torch.tensor(result, device=read_data_list.device)
    ...
    def forward(self, obs_data_list, seq_size, init_size):
        ...
        ##############################
        # masked observation encoder #
        ##############################
        masked_enc_obs_list = list()
        with torch.no_grad():
            for i in range(1, obs_data_list.size()[2]-1):
                masked_obs = obs_data_list.detach()
                masked_obs[:, :, i] = 0
                masked_enc_obs = self.enc_obs(masked_obs.view(-1, *obs_data_list.size()[2:]))
                masked_enc_obs = masked_enc_obs.view(num_samples, full_seq_size, -1)
                masked_enc_obs_list.append(masked_enc_obs)
        ...
        ######################################
        # (6) forward imagined masked states #
        ######################################
        batch_boundary = self.random_index_from_ones(boundary_data_list[:,:,0], init_size, seq_size)
        masked_pred_lists = [[] for _ in range(obs_data_list.size(2)-2)]
        # TODO: compute correct abs_feat
        # use post_abs_state_list and abs_belief (how to compute abs_belief?)
        # TODO2: self.post_boundary to compute difference between masked and unmasked
        with torch.no_grad:
            for i in range(obs_data_list.size(2)-2):
                # masked_obs_post_fwd -> masked_post_obs_state -> masked_obs_state -> masked_obs_feat -> masked_pred
                masked_obs_post_fwd = obs_data_list.new_zeros(obs_data_list.size(2)-2, num_samples, self.obs_belief_size)
                masked_pred = obs_data_list.new_zeros(obs_data_list.size(2)-2, num_samples, *obs_data_list.size()[2:])
                t_init = batch_boundary-init_size
                for t in range(init_size):
                    masked_enc_obs = masked_enc_obs_list[i][torch.arange(num_samples), t_init+t]
                    masked_enc_pred = self.enc_obs(masked_pred[i])
                    masked_encs = masked_enc_obs if t==0 else masked_enc_pred
                    masked_obs_post_fwd[i] = self.obs_post_fwd(masked_encs, masked_obs_post_fwd[i])
                    masked_post_obs_state = self.post_obs_state(concat(masked_obs_post_fwd, abs_feat))
                    masked_obs_state = masked_post_obs_state.rsample()
                    masked_obs_feat = self.obs_feat(concat(obs_belief, masked_obs_state))
                    masked_pred[i] = self.dec_obs(masked_obs_feat)
                    masked_pred_lists[i].append(masked_pred[i])
                masked_obs_post_fwd = obs_data_list.new_zeros(obs_data_list.size(2)-2, num_samples, self.obs_belief_size)
                masked_pred = obs_data_list.new_zeros(obs_data_list.size(2)-2, num_samples, *obs_data_list.size()[2:])
                t_init = batch_boundary
                for t in range(init_size):
                    masked_enc_obs = masked_enc_obs_list[i][torch.arange(num_samples), t_init+t]
                    masked_enc_pred = self.enc_obs(masked_pred[i])
                    masked_encs = masked_enc_obs if t==0 else masked_enc_pred
