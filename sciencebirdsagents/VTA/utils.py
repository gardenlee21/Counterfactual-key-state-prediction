...
def postprocess(image, bits=5, idx=0):
    bins = 2 ** bits
    image = image / 2.0 + 0.5
    image = torch.floor(bins * image)
    image = image * (255.0 / (bins - 1))
    image = torch.clamp(image, min=0.0, max=255.0) / 255.0
    # TODO: transform from channel 6 to 3 for visualization
    # channels 0 (red), 1 (green), 2 (brown), 3 (blue), 4 (gray), 5 (black)
    # 0=bird, 1=pig, 2=wood, 3=ice, 4=stone, 5=platform
    device = torch.device('cuda', idx)
    image_rgb = torch.ones(image.size(0), image.size(1), 3, image.size(3), image.size(4)).to(device)
    image_rgb[:, :, 1] -= image[:, :, 0]        # bird
    image_rgb[:, :, 2] -= image[:, :, 0]        # bird
    image_rgb[:, :, 0] -= image[:, :, 1]        # pig
    image_rgb[:, :, 2] -= image[:, :, 1]        # pig
    image_rgb[:, :, 0] -= image[:, :, 2] * 0.36 # wood
    image_rgb[:, :, 1] -= image[:, :, 2] * 0.84 # wood
    image_rgb[:, :, 2] -= image[:, :, 2] * 0.84 # wood
    image_rgb[:, :, 0] -= image[:, :, 3]        # ice
    image_rgb[:, :, 1] -= image[:, :, 3]        # ice
    image_rgb[:, :, 0] -= image[:, :, 4] *0.5   # stone
    image_rgb[:, :, 1] -= image[:, :, 4] *0.5   # stone
    image_rgb[:, :, 2] -= image[:, :, 4] *0.5   # stone
    image_rgb[:, :, 0] -= image[:, :, 5]        # platform
    image_rgb[:, :, 1] -= image[:, :, 5]        # platform
    image_rgb[:, :, 2] -= image[:, :, 5]        # platform
    return image_rgb
...
def full_dataloader(seq_size, init_size, batch_size, test_size=16, data_path='./vta_3dmaze_32x32_1000000.npz'):
    import sys, os; from os import path; import logging
    sys.path.append(path.dirname(path.dirname(path.abspath(__file__))))
    from LearningAgents.MemoryDataset import VTADataset
    logger = logging.getLogger("VTA training")
    memory_saving_path = os.path.join(os.getcwd(), 'LearningAgents', 'saved_memory')
    trainset = VTADataset(h=64, w=96, c=6, 
                        path=memory_saving_path, logger=logger, max_len=100000)
