...
class Encoder(nn.Module):
    def __init__(self,
        ...
        network_list = [ConvLayer2D(input_size=6,
                                    output_size=feat_size,
                                    kernel_size=4,
                                    stride=2,
                                    padding=1),  # 32 x 48
        ...
        if output_size is not None:
            network_list.append(LinearLayer(input_size=feat_size*6,
                                            output_size=feat_size))
...
class Decoder(nn.Module):
    ...
    def forward(self, input_data):
        return self.network(self.linear(input_data).reshape([-1, self.feat_size, 2, 3]))
